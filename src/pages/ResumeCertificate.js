...
export default class User extends Component {
	...
	render() {
		return(
			<View>
	    		<NavigationBar title='所获证书' navigation={this.props.navigation} />
	    		<ScrollView>
			      <View style={styles.container}>
			      	<View style={styles.formWrap}>
								<View style={[styles.base,styles.baseTop]}>
									<View style={[styles.item,styles.itemTop]}>
										<View style={styles.itemTitle}>
											<Text style={styles.itemTitleText}>获得时间</Text>
										</View>
										<_Picker ref="getTime" title='' style={styles.itemPicker}
										    cback={(data)=>this.setForm('getTime',data)}
										    type="getTime"
										/>
									...
									<View style={styles.item}>
										<View style={styles.itemTitle}>
											<Text style={styles.itemTitleText}>所获证书</Text>
										</View>
										<TextInput style={styles.itemInput}
											onChangeText={(data) => this.setForm('certName',data)}
											value={this.state.formInfo.certName} placeholder="请填写" 
				          ...
				          <Text style={{color: commonStyle.themeColor}}>删除此所获证书</Text>
          ...
          dialogTitle={<DialogTitle title='是否确认删除所获证书' />}
          width={280}
          height={35+50}
        >
		...
		if(this.state.itemId) {
			...
			ajaxResume.getExperience(config, res => {
				if(res.code == 200) {
					...
					formInfo.getTime = resume.getTime || '';
					formInfo.certName = resume.certName;

					this.setState({
						formInfo,
					})

					this.refs.getTime.serTitle(resume.getTime||'');
				} else {
	...
	basicSubmit() {
		...
		const config = {
			certName: this.state.formInfo.certName,
			getTime: this.state.formInfo.getTime,
		...
		ajaxResume.addCertificate(config, res => {
			if(res.code == 200) {
	...
	_checkForm(data) {
		if(data.getTime === '') {
			this.refs.toast.show('请选择获得时间');
			return false
		}else if(!data.certName) {
			this.refs.toast.show('请填写所获证书');
			return false
		}
		return true
	}
